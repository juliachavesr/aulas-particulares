...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventOverlap: false,
        dateClick: function(info) {
            // Ajusta o intervalo para incluir pequenos desvios no clique
            var startBoundary = new Date(info.date.getTime() - 30 * 1000); // 30 segundos antes
            var endBoundary = new Date(info.date.getTime() + 30 * 1000); // 30 segundos depois

            // Verifica se o horário clicado está dentro de um slot já selecionado
            var existingSlot = selectedSlots.find(slot => 
                slot.start.getTime() >= startBoundary.getTime() && 
                slot.start.getTime() <= endBoundary.getTime()
            );

            if (existingSlot) {
                // Remove o horário selecionado
                selectedSlots = selectedSlots.filter(slot => slot !== existingSlot);
                var event = calendar.getEvents().find(e => e.start.getTime() === existingSlot.start.getTime());
                if (event) event.remove();
            } else {
                // Adiciona o horário selecionado
                selectedSlots.push({
